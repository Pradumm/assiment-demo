import { RxCross1 } from "react-icons/rx";

import { useMyContext } from '../component/store/ContextApi';


const CardItem = ({ user, RemoveUser }) => {

    const { toggleItem, toglegridview } = useMyContext()


    return (
        <div class="flex relative items-center  gap-2">


            <div className={` ${toggleItem ? " flex  items-center flex-col sm:flex-row h-full xl:h-36" : 
            " flex-col xl:h-[470px]"} bg-white px-5 py-3 
             shadow-sm mb-7 rounded-xl flex  items-center gap-5 `}>
                <div className={` ${toggleItem ? "flex flex-shrink-0" : "hidden"} `}>
                    <img
                        className='w-16 h-16 rounded-full'
                        src='data:image/jpeg;base64,'
                        alt="" />
                </div>
                <div>
                    <h1 className='text-xl capitalize hidden sm:block font-semibold text-black'>

                        {user?.title.split(' ').slice(0, 15).join(' ')}
                        {user?.title.split(' ').length > 20 ? ' ...' : ''}


                    </h1>


                    <h1 className='text-xl block sm:hidden font-semibold text-black'>

                        {user?.title.split(' ').slice(0, 10).join(' ')}
                        {user?.title.split(' ').length > 20 ? ' ...' : ''}


                    </h1>


                    <p className='text-lg   hidden sm:block font-normal text-black my-1'>
                        {user.body.split(' ').slice(0, 15).join(' ')}
                        {user.body.split(' ').length > 20 ? ' ...' : ''}
                    </p>

                    <p className='text-lg  block sm:hidden font-normal text-black my-1'>
                        {user.body.split(' ').slice(0, 10).join(' ')}
                        {user.body.split(' ').length > 20 ? ' ...' : ''}
                    </p>

                    <p className='text-base text-gray-400 font-semibold'>Mon , 21 Dec 2020 12:57 GMT</p>

                </div>


                <img
                    className={`${toggleItem == false ? "w-full h-36 object-cover" : "hidden"}`}
                   src="data:image/jpeg;base64,"
                    alt=""
                />




            </div>


            <div className={`${toggleItem == false ? "absolute right-0 top-0" : ""} lg:-mt-3  bg-white p-2 flex items-center justify-center rounded-full`}>
                <RxCross1
                    onClick={() => RemoveUser(user.userId)}

                    className="w-8 h-8 text-red-700" />
            </div>


        </div >

        
    )

}


export default CardItem